...
os.getcwd()
os.chdir('./Desktop/merge')

kospi_defacto = pd.read_csv('./kospi_net/000020.csv', sep=',')
kospi_ohlcv = pd.read_csv('./kospi_ohlcv/000020.csv', sep=',', encoding='CP949')
kospi_buy = pd.read_csv('./kospi_buy/000020.csv', sep=',')
...
for agent in ['ind', 'for', 'ins', 'cor']:
    kospi_defacto[agent + '_n_p'] = kospi_defacto[agent + '_n'] + abs(kospi_defacto[agent + '_n'].min())
    kospi_defacto[agent + '_n_p_cumsum'] = kospi_defacto[agent + '_n_p'].cumsum()
    kospi_defacto[agent + '_tp*n_p'] = kospi_defacto[agent + '_tp'] * kospi_defacto[agent + '_n_p']
    kospi_defacto[agent + '_tp*n_p_cumsum'] = kospi_defacto[agent + '_tp*n_p'].cumsum()
    kospi_defacto[agent + '_apps_tp'] = kospi_defacto[agent + '_tp*n_p_cumsum']/kospi_defacto[agent + '_n_p_cumsum']

drop_list = ['individual', 'foreign_retail', 'institution', 'trust', 'pension', 'etc_corporate', 'open_price', 'high_price', 'low_price', 'volume', 'individual_buy', 'foreign_retail_buy', 'institution_buy', 'etc_corporate_buy',]
...
kospi_defacto.index.strftime('%Y-%m')
kospi_defacto['2018-02'].shape[0]
...
kospi_defacto['institution_tvalue_section'] = np.where(kospi_defacto['institution_tvalue']>ih_Q3,4,np.where(kospi_defacto['institution_tvalue']>ih_Q2,3,np.where(kospi_defacto['institution_tvalue']>ih_Q1,2,1)))
kospi_defacto['foreign_retail_tvalue_section'] = np.where(kospi_defacto['foreign_retail_tvalue']>ip_Q3,4,np.where(kospi_defacto['foreign_retail_tvalue']>ip_Q2,3,np.where(kospi_defacto['foreign_retail_tvalue']>ip_Q1,2,1)))
kospi_defacto['institution_coef_section'] = np.where(kospi_defacto['institution_coef']>ih_Q3,4,np.where(kospi_defacto['institution_coef']>ih_Q2,3,np.where(kospi_defacto['institution_coef']>ih_Q1,2,1)))
kospi_defacto['foreign_retail_coef_section'] = np.where(kospi_defacto['foreign_retail_coef']>ip_Q3,4,np.where(kospi_defacto['foreign_retail_coef']>ip_Q2,3,np.where(kospi_defacto['foreign_retail_coef']>ip_Q1,2,1)))
...
kospi_defacto.to_csv('000020.csv')
